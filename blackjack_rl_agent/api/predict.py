...
def cards_to_state(opponents_card, player_cards):
    ...
    state = (player_hand_value, opponents_card, is_soft)

    return state, double_poss, split_poss


def best_move(opponents_card, *player_cards):
    actions_dic = {0: "stand", 1: "hit", 2: "double", 3: "split"}
    model = load_model()

    state, double_poss, split_poss = cards_to_state(opponents_card, player_cards)
